...
- **Exact-form display**: on by default (`fracDisplay = true`). After calculation the result is shown in exact form when possible — a fraction with denominator ≤ 999 (`n/d`), a π multiple (`5π`), or a simplified surd (`5√2`). Improper fractions are the default; `fracMixed` switches the result to a mixed number (`w n/d`). The `< >` key (`toggle-display`) flips between this exact form and the decimal approximation.
- **Fraction entry (`n/d`)**: never reuses `Ans`. Pressing `n/d` enters fraction mode — if a value sits to its left, only the *immediately preceding operand* (a number, constant, or a `(…)`/function group, via `_operandStart`) becomes the numerator, so `4/5 + 6 n/d 5` = `4/5 + 6/5` (not `(4/5+6)/5`); otherwise it inserts an empty `▯/▯` template with the caret in the numerator (type the numerator there to build a complex one like `(5+6)/3`). Operators typed inside a box stay in that box (so `5 n/d 2 + 6` = `5/(2+6)`), and the same applies to the `^` exponent box.
- **Mixed-number entry (`2nd + n/d`, `U n/d`)**: type the whole unit, press `2nd + n/d`, then the numerator, `↓`, the denominator (e.g. `1 [2nd n/d] 7 ↓ 12` = `1 7/12`). Internally a mixed number is the atomic group `( U mixed (n) frac (d) )` evaluated as `U + n/d` (the `mixed` operator carries the unit's sign); the display renders the unit adjacent to the stacked fraction with no `+`, and `→` exits the whole template in one press.
- **n/d ↔ U n/d conversion (`2nd + ×10ⁿ`)**: toggles `fracMixed`, i.e. flips the result between improper-fraction and mixed-number form.
